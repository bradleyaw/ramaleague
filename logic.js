import players from '/mPlayers.js';
const rankPlayers = (playerArray) => {
    return playerArray.sort(function (a, b) {  return b.wins - a.wins || b.points - a.points })
}

const resultsOne = [
        {id: 1, wins: 2, losses: 1, points: 6},
        {id: 2, wins: 2, losses: 1, points: 4},
        {id: 3, wins: 3, losses: 0, points: 8},
        {id: 4, wins: 1, losses: 2, points: 3},
        {id: 5, wins: 2, losses: 1, points: 7},
        {id: 6, wins: 1, losses: 2, points: 2},
        {id: 7, wins: 0, losses: 3, points: 0},
        {id: 8, wins: 2, losses: 1, points: 5},
        {id: 9, wins: 2, losses: 1, points: 9},
        {id: 10, wins: 0, losses: 0, points: 0},
        {id: 11, wins: 0, losses: 0, points: 0},
        {id: 12, wins: 0, losses: 0, points: 0},
        {id: 13, wins: 0, losses: 0, points: 0},
        {id: 14, wins: 0, losses: 0, points: 0},
        {id: 15, wins: 0, losses: 0, points: 0}
]
resultsOne.map((player, index) => {
    players[index].wins += player.wins;
    players[index].losses += player.losses;
    players[index].points += player.points;
    });

const pContent = document.querySelector('#playerContent');
const rList = document.querySelector('#resultList');
const playersRanked = rankPlayers(players);
const pItems = playersRanked.map(player => `<tr>
                                        <th scope="row">1</th>
                                        <td>${player.name}</td>
                                        <td>${player.wins}</td>
                                        <td>${player.losses}</td>
                                        <td>${player.points}</td>                
                                    </tr>`)
console.log(pItems);
pContent.innerHTML = pItems.join('');
